...
if (btn_preview) {
  btn_preview.innerText = "retour au portail du site";
  btn_preview.setAttribute(
    "href",
...
for (i = 0; i < theme.h1_list.length; i++) {
  test = theme.h1_list[i];
  // console.log(test);
  if (test.innerText.indexOf("Portail") != -1) {
    // console.log(true + " " + "contient portail");
    theme.h1_list = [Array.from(theme.h1_list).pop()];
  }
}
...
add_class_to_element_list(
  theme.p_list,
  theme.class_list_p.concat(["pt-5", "pb-3"])
);
/** =======================================================
...
if (divbtt) {
  ...
  window.addEventListener("scroll", function (event) {
    ...
    divbtt.className = scrollPos > scrollMax * 0.7 ? "visible" : "invisible";
    if (!isFirefox) {
      divbtt.className = scrollPos > docHeight * 0.5 ? "visible" : "invisible";
    }
...
theme.add_class_to_element_list(
  [badge],
  class_badge_position.concat(theme.class_color_theme)
);
...
theme.add_class_to_element_list(
  [carousel_name],
  ["bg", "bg-cyan-800", "rounded", "text-cyan-200", "p-2", "mt-5"]
);
...
function delay_x(element_x, className, dtime) {
  setTimeout(function () {
    element_x.classList.remove(className);
  }, dtime);
}
...
redirect_link1.classList.add(
  theme.mybadge_color1,
  "bg",
  "bg-primary",
...
function get_background_number() {
  return backgrounds.indexOf(slide_preview.style.backgroundImage);
}
...
function Animations() {
  ...
  tl2
    .fromTo(
      ...
      1,
      { width: "0%" },
      { width: "100%", ease: Bounce },
      "-=1"
    )
    .fromTo(badge, 3, { width: "0%" }, { width: "30%" })
    .fromTo(badge, 3, { opacity: 0 }, { opacity: 1 }, "-=3")
    .to(img_container, 1, {
      opacity: 1,
      scrollTrigger: {
...
function customize() {
  ...
  TweenMax.fromTo(
    ...
    2,
    { x: 0, rotation: 0 },
    { x: "10vw", rotation: 360, onComplete: triggerToast }
  );
}
...
function Rotation(elem, t1, angle1 = 0, angle2 = 360, delay = 0) {
  TweenMax.fromTo(elem, t1, { rotation: angle1 }, { rotation: angle2 }).delay(
    delay
  );
}
...
function addAnimation(elements_list, delay = 0) {
  for (i in elements_list) {
    ...
    function handler(event) {
      let t1;
      var tweenRotation = TweenMax.fromTo(
        event.target,
        (t1 = 0.5),
        { rotation: (angle1 = 0) },
        {
          rotation: (angle2 = (-1) ** Math.ceil(Math.random() + 1) * 20),
          ease: Power2.easeInOut,
        }
      ).delay(0);
      if (event.type == "mouseenter") {
        // cette condition ne sert à rien car l'animation de rotation
        //se déclenche de toute façon à chaque event.
        // console.log('test')
      }
      if (event.type == "mouseout") {
        // celle-ci en revanche est nécessaire
...
function add_class_to_element_list(
  list = [],
...
theme.myScrollTrigOpacity(
  [text_container, journal],
  main_container,
  "400px 70%",
  "700px 20%"
);
...
//   )
//
let close_video = document.querySelector("#close_button");
var player;
var tag = document.createElement("script");
tag.src = "https://www.youtube.com/iframe_api";
tag.id = "youtube_player";
// let youtube_player = document.querySelector("#player").appendChild(tag)
var firstScriptTag = document.getElementsByTagName("script")[17];
